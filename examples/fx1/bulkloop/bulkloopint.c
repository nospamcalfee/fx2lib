...
{
	static uint8_t ep2count;

	/* clear interrupt */
	EXIF &= ~USBINT;
	OUT07IRQ = OUT2IR;
	/* transfer out as next in buffer */
	/* Reload the buffers. */
	IN2BUF[0] = USBFRAMEL;
	IN2BUF[1] = USBFRAMEH;
	IN2BUF[2] = ep2count++;
	memcpy(&IN2BUF[3], OUT2BUF, MAX_PACKET_BULK_FS - 3);
	IN2BC = MAX_PACKET_BULK_FS;
	/* Re-arm the out ep too */
	OUT2BC = MAX_PACKET_BULK_FS;
}
